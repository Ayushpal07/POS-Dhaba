...
export default function Billing({ bills, orders, setMsg, onRefresh }) {
  ...
  const paid = selectedBill?.status === 'PAID';
  ...
  async function cancelOrder() {
    ...
    try {
      ...
      setMsg(`Order #${order.id} cancelled. Table ${order.table_number} is available.`);
      setSelectedBillId(null);
      await onRefresh();
    } catch (error) {
      setMsg(error.message);
  ...
  async function collectPayment() {
    if (!selectedBill || paid || busy) return;
    setBusy(true);
    try {
      await api('/api/payments/', {
        method: 'POST',
        body: JSON.stringify({ bill: selectedBill.id, amount: selectedBill.total, method: paymentMethod }),
      });
  ...
  return <section className="page">
    <div className="head">
      <div>
        <h2>Billing</h2>
        <p>Review every bill, inspect the complete table order and close the table when finished.</p>
      </div>
    </div>
    ...
    <div className="stats">
      ...
      <div className="stat"><small>Awaiting payment</small><strong>{issued.length}</strong><span>Tables waiting</span></div>
      <div className="stat"><small>Gross value</small><strong>{money(gross)}</strong><span>All bills</span></div>
    </div>

    <div className="panel table-wrap">
      <table>
        <thead><tr><th>Bill</th><th>Table</th><th>Order</th><th>Subtotal</th><th>Total</th><th>Status</th><th>Issued</th></tr></thead>
        <tbody>
          {bills.map(bill => <tr key={bill.id} onClick={() => setSelectedBillId(bill.id)} style={{ cursor: 'pointer' }}>
            <td>#{bill.id}</td>
            ...
            <td>{dt(bill.issued_at || bill.created_at)}</td>
          </tr>)}
        </tbody>
      </table>
    ...
    {selectedBill && <div className="modal-bg" onMouseDown={() => !busy && setSelectedBillId(null)}>
      <div className="modal bill-detail" onMouseDown={event => event.stopPropagation()}>
        <div className="modal-head">
          <div><h2>Bill #{selectedBill.id}</h2><small>Table {selectedBill.table_number} · Order #{selectedBill.order}</small></div>
          <button onClick={() => setSelectedBillId(null)} disabled={busy}>×</button>
        </div>

        <div className="bill-summary">
          <div><span>Order status</span><Status value={order?.status || selectedBill.order_status} /></div>
          <div><span>Bill status</span><Status value={selectedBill.status} /></div>
          <div><span>Table</span><b>Table {selectedBill.table_number}</b></div>
        </div>

        <div className="bill-history">
          <div className="panel-head"><div><h3>Complete order history</h3><small>Every round served to this table</small></div></div>
          {order?.rounds?.map(round => <div className="round" key={round.id}>
            <div className="round-head"><b>Round {round.number}</b><small>{dt(round.sent_at || round.created_at)}</small></div>
            {round.items?.map(item => <div className="order-line" key={item.id}>
              <div><strong>{item.menu_item_name}</strong>{item.notes && <small>{item.notes}</small>}</div>
              <span>{item.quantity} × {money(item.unit_price)}</span>
              <b>{money(item.line_total ?? Number(item.quantity) * Number(item.unit_price))}</b>
            </div>)}
          </div>)}
        ...
        <div className="bill-lines">
          <p>Subtotal <b>{money(selectedBill.subtotal)}</b></p>
          <p>Tax <b>{money(selectedBill.tax)}</b></p>
          <p>Discount <b>-{money(selectedBill.discount)}</b></p>
          <p className="bill-grand">Total <b>{money(selectedBill.total)}</b></p>
        </div>

        {!paid && order && order.status !== 'CANCELLED' && <>
        ...
        {order?.status === 'CANCELLED' && <div className="error">This order was cancelled and the table has been released.</div>}
      </div>
    </div>}
  </section>;
}
